package com.dtwo.rpgaction.utils;

public class AppConstants {

    public static final String USER_CREATION_SUCCESS = "Usuario criado com sucesso! id - %s, username - %s";

    public static final String GENERIC_CONTROLLER_ERROR = "Erro ao executar chamada ao controller - ";

    public static final String GENERIC_SCOPE_ERROR = "Erro ao executar scope - ";
    public static final String PAYLOAD_SCOPE_ERROR = "Não foi possível processar payload.";

    public static final String GENERIC_VALIDATION_ERROR = "Erro na validação - ";
    public static final String USER_VALIDATION_USERNAME_ERROR = "Username não pode estar vazio.";
    public static final String USER_VALIDATION_PASSWORD_ERROR = "Password não pode estar vazio.";
    public static final String USER_VALIDATION_EMAIL_ERROR = "Email não pode estar vazio.";
    public static final String USER_USERNAME_EXISTS_ERROR = "Nome de usuário já existe.";

    public static final String GENERIC_ACTION_ERROR = "Erro ao processar informações - ";
    public static final String USER_ACTION_SAVE_USER_ERROR = "Erro ao salvar novo usuário.";

    public static final String USER_DOES_NOT_EXISTS = "Não existem usuários cadastrados com esse username: %s.";
    public static final String PASSWORD_IS_NOT_VALID = "Senha inválida";


}
